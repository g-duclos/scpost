...
* **ExpressionSet.rds** (S4 object containing GEX counts + cells & feature metadata)
...
* Data integration intended to correct for technical batch effects for downstream clustering & dimensionality reduction, based on a selected "batch" variable of interest using one of the following methods:
	...
	* [Seurat's "RPCA" method](https://satijalab.org/seurat/articles/integration_rpca.html)(**please note - I am in the processing of updating to reflect a recently simplified approach to coding this method in Seurat v5**)

* A single command to perform unsupervised cell clustering using Seurat's standard methodology (SNN _ Louvain community detection) with the option of returning a range of cluster solutions.
...
* The Seurat *RunUMAP* function is used to run UMAP on the dataset and results are stored as a list labeled "tSNE" in the "assayData" slot labeled "Seurat" in the ExpressionSet object. According to the number of iterations specified in the annotation file, results are subsequently stored as "Iteration: 1", "Iteration: 2", ...
